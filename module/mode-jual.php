...
function generateNo(){
    ...
    $queryNo = mysqli_query($koneksi, "SELECT max(no_jual) as maxno 
    FROM tbl_jual_head");
...
function insert ($data) {
    ...
    $cekbrg = mysqli_query($koneksi, "SELECT * FROM tbl_jual_detail WHERE no_jual =
    '$no' AND barcode = '$kode'");
    ...
    if (empty($qty)) {
        ...
        $sqlJual = "INSERT INTO tbl_jual_detail VALUE (null, '$no', '$tgl', 
        '$kode','$nama', $qty, $harga, $jmlharga)";
        mysqli_query($koneksi, $sqlJual);
    }

    mysqli_query($koneksi, "UPDATE tbl_barang SET stock = stock - $qty WHERE barcode
    = '$kode'");
