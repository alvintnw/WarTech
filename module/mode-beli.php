...
function generateNo(){
    ...
    $queryNo = mysqli_query($koneksi, "SELECT max(no_beli) as maxno 
    FROM tbl_beli_head");
...
function insert ($data) {
    ...
    if (mysqli_num_rows($cekbrg)) {
        echo "<script>
                alert('barang sudah ada anda harus menghapusnya dulu jika ingin 
                mengubah qty nya..');
        </script>";
        return false;
    }
    ...
    if (empty($qty)) {
       echo "<script>
        ...
        $sqlbeli = "INSERT INTO tbl_beli_detail VALUE (null, '$no', '$tgl', 
        '$kode','$nama', $qty, $harga, $jmlharga)";
        mysqli_query($koneksi, $sqlbeli);
    }

    mysqli_query($koneksi, "UPDATE tbl_barang SET stock = stock + $qty WHERE id_barang 
    = '$kode'");
...
function simpan($data){
    ...
    $sqlbeli = "INSERT INTO tbl_beli_head VALUE ('$nobeli', '$tgl', '$suplier', '$total', '$keterangan')";
    mysqli_query($koneksi, $sqlbeli);
